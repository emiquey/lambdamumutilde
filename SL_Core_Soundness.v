...
Require Import LibLN 
  ...
  L_Core_Soundness.
Require Import SL_Core_Definitions.
...
Fixpoint typing_sprf_weaken A tau p T (H: typing_sprf A tau p T) {struct H}:
  forall E F G, A=E&G -> ok (E & F & G) ->  typing_sprf  (E & F & G) tau p T
with typing_scont_weaken A tau e T (H:typing_scont A tau e T) {struct H}:
  forall E F G, A=E&G ->  ok (E & F & G) -> typing_scont (E & F & G) tau e T
with typing_sclos_weaken A tau c  (H: typing_sclos A tau c) {struct H}:
  forall E F G, A=E&G -> ok (E & F & G) -> typing_sclos (E & F & G) tau c .
Proof.
  induction H; intros.
  + apply* typing_sprf_var.
    apply* binds_weaken.
    subst.
    assumption.
  + apply typing_store_var.
    destruct H as (q,(l,(Hl,(Binds,HI)))).
    exists q l;repeat split*.
 ...
 +  apply_fresh (@typing_mu). 
    rewrite <- concat_assoc.
    apply (typing_sclos_weaken (E0 & (G & y ~ neg T))).
    rewrite  concat_assoc.
    subst.
    apply* H.
    reflexivity.
 ...
 +
    ...
    Guarded.
Lemma typing_store_weaken :
  (  forall A tau p T (H: typing_sprf A tau p T) E F G,
       A=E&G -> 
       ok (E & F & G) ->  typing_sprf  (E & F & G) tau p T)
/\ (forall A tau e T (H:typing_scont A tau e T) E F G,
A=E&G ->  ok (E & F & G) -> typing_scont (E & F & G) tau e T)
/\ (forall A tau c  (H: typing_sclos A tau c) E F G,
 A=E&G -> ok (E & F & G) -> typing_sclos (E & F & G) tau c) .
Proof.
  apply (typing_store_mut_ind).
  + intros A tau a T OkA Binds E F G HA Ok.
    apply* typing_sprf_var. apply* binds_weaken.
    rewrite HA in Binds.
    assumption.
  + intros A tau a T (q,(l,(Hl,(Binds,Hq)))) E F G HE Ok.
    apply* typing_store_var.
    exists q l;repeat split*. 
    apply* binds_weaken.
    rewrite HA in Binds.
